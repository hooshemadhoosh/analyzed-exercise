...
for student_number in data:
    htmlfile = open('sport-program-build\index.html' , 'r' , encoding='utf8')
    index = htmlfile.read()
    soup = BeautifulSoup(index , 'html.parser')

    #------general information------------
    age = 402 - int(str(student_number)[:3]) + 18
    bmi = str(data[student_number]['BMI'])

    #finding the paragraph tag which contains the name of the person
    nameee = soup.find('p' , {'id' : 'name-content'})

    #changing the p tag where it contains some letters and replace it with the string form of the student_number!
    new_text = nameee.find(text=re.compile(r'^[\u0600-\u06FF\s]+$')).replace_with(str(student_number))
    ...
    #JUNK CODES! YOU MAY NEED THEM!
    #age = nameee[1] 
    #name = nameee[3]
    #name = BeautifulSoup(str(name) , 'html.parser').p
    #name.string = str(student_number)
    student = data[student_number]
    for i in range(5):
        ind_pr = random.randint(0,2)
        exc_program = student['Program'][ind_pr]
        ...
        for exercise in exercises:
            ...
            row_html_str ="""<div class="space-y-2 pt-4 tbl-row">
                                <div class="space-y-2">
            ...
            if len(exercise[1]) > 0:
                for i in exercise[1]:
                    str_img_tag = make_img_tag('../Whole_body/Beginner/' + str(i))
            ...
            commit_exer_name = row.find('span' , {'id':"exercise-name"}).find(text=re.compile(r'^[\u0600-\u06FF\s]+$')).replace_with(str(exercise[0]))
    ...
    with open("sport-program-build\\" + f'newFile{student_number}.html' , 'wb') as f:
